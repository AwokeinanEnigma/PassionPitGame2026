...
namespace PassionPitGame {
    public class AIMotor : Motor, ICharacterController {
        #region  Enums
        ...
        private void GroundMove(ref Vector3 currentVelocity)
        {
            ...
            Vector3 reorientedInput = Vector3.Cross(KMotor.GroundingStatus.GroundNormal, inputRight).normalized *
        ...
        private void AirMove(ref Vector3 currentVelocity)
        {
            ...
            if (WishDirection.sqrMagnitude > 0f)
            {
                ...
                if (KMotor.GroundingStatus.FoundAnyGround)
                    if (Vector3.Dot(currentVelocity + addedVelocity, addedVelocity) > 0f)
                    {
                        Vector3 perpenticularObstructionNormal = Vector3
                            .Cross(Vector3.Cross(Up, KMotor.GroundingStatus.GroundNormal),
                                Up).normalized;
        ...
        private void ApplyGravity()
        {
            ...
            position.y -= KMotor.Capsule.height * 0.5f;
        ...
        public override void SetWishDirection(Vector3 direction)
        {
        ...
        private Vector3 PlanarAccelerate(Vector3 currentVelocity, Vector3 wishDirection, float accelerationRate,
            float accelerationLimit)
        {
            float speed = Vector3.Dot(Vector3.Scale(KMotor.GroundingStatus.GroundNormal, currentVelocity),
                Vector3.Scale(wishDirection,  KMotor.GroundingStatus.GroundNormal).normalized);
            float speedGain = accelerationRate * Time.deltaTime;
            ...
            switch (CurrentCharacterState)
            {
                    ...
                    else if (CurrentBonusOrientationMethod == BonusOrientationMethod.TowardsGroundSlopeAndGravity)
                    {
                        if (KMotor.GroundingStatus.IsStableOnGround)
                        {
                            Vector3 initialCharacterBottomHemiCenter =
                                KMotor.TransientPosition + Up * KMotor.Capsule.radius;
                ...
                case MotorState.Planet:
                    ...
                    else if (CurrentBonusOrientationMethod == BonusOrientationMethod.TowardsGroundSlopeAndGravity)
                    {
                        ...
                        {
                            Vector3 initialCharacterBottomHemiCenter =
                                KMotor.TransientPosition + currentUpP * KMotor.Capsule.radius;
        ...
        public void UpdateVelocity(ref Vector3 currentVelocity, float deltaTime)
        {
            ...
            DrawVector(KMotor.TransientPosition, Up, 25, Color.red);
            DrawVector(KMotor.TransientPosition, -Up, 25, Color.blue);
        }
        ...
        public void BeforeCharacterUpdate(float deltaTime)
        {
            ...
            int hitShit = KMotor.CharacterCollisionsSweep(KMotor.TransientPosition, KMotor.TransientRotation,
        ...
        protected void OnLanded()
        {
            ...
            _currentSurfaceCollider = KMotor.GroundingStatus.GroundCollider;
            OnGrounded?.Invoke();
